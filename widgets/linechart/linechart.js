...
SZN.LineChart.prototype._draw = function() {
	...
	var availw = this.width - (o.padding + this.offsetLeft + lw);
	this.interval = Math.round(availw / (length - 1));
	...
	if (this.options.rows) { /* horizontal lines */
		...
		for (var i=min;i<=max;i+=result) {
			...
			new SZN.Vector.Line(this.canvas, [new SZN.Vec2d(this.offsetLeft, top), new SZN.Vec2d(this.offsetLeft+availw, top)], style)
			idx++;
		}
...
SZN.LineChart.prototype._drawLegend = function() {
	...
	for (var i=0;i<this.data.length;i++) {
		...
		var l = w+3*o.padding+o.legendWidth+10;
