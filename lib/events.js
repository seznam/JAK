...
SZN.Events._domReadyAlreadyRun = false;
...
SZN.Events._domReady = function () {
	...
	if (document.addEventListener) {
		document.removeEventListener("DOMContentLoaded", SZN.Events._domReady, true); alert('remove');
	}
...
/**
 * metoda kterou použijeme, pokud chceme navěsit vlastní kód na událost na stav, kdy je DOM připraven k použití  
 * @method 
...
SZN.Events.onDomReady = function(obj, func) {
	...
	if((/Safari/i.test(navigator.userAgent)) || (/WebKit|Khtml/i.test(navigator.userAgent))){ //safari, konqueror
		...
		//zapiseme do dokumentu JS ktery bude nacteno az po dokumentu diky defer tagu
  	...
  	//SZN.Events.addListener(window, 'load', window, function(){alert('onload');setTimeout("if (!SZN.Events._domReadyAlreadyRun) {SZN.Events._domReady();}", 0);});
  	
	/*window.onload=function(){
		setTimeout("if (!alreadyrunflag) walkmydog()", 10);
	}*/
}
