...
String.prototype.localeCSCompare = function(value) {
	...
	var stringLong = this.length > value.length ? this : value;
	var stringShort = this.length > value.length ? value : this; 
	var i = 0;
	var j = 0;
	var len = stringShort.length;
	var charLong = '';
	var charShort = '';
	var indexLong = 0;
	var indexShort = 0;
	...
	while(j < len) {
		...
		charLong = stringLong.charAt(i);
		charShort = stringShort.charAt(j);

		/* c je podezrely znak, protoze po nem muze nasledovat h a mame najednou znak ch */
		if (charLong.toLowerCase() == 'c') {
			var tempString = stringLong.substring(i, i + 2);
		...
		if (j + 1 < len && charShort.toLowerCase() == 'c') {
			var tempString = stringShort.substring(j, j + 2);
			if (tempString == "ch" || tempString == "CH") {
				j++;
				charShort = tempString;
			}
		}

		/* zjistime si, kde se v nasi abecede nachazi */
		indexLong = this.CS_ALPHABET.indexOf(charLong);
		indexShort = this.CS_ALPHABET.indexOf(charShort);

		/* pokud jsme narazili na ruzne znaky, koncime */
		if (charLong != charShort) { break; }
	...
	if (stringLong == this) {
		if (indexLong == indexShort) { return 1; }
		if (indexLong == -1) { return -1; }
		if (indexShort == -1) { return 1; }
		return indexLong - indexShort;
	} else {
		if (indexLong == indexShort) { return -1; }
		if (indexShort == -1) { return -1; }
		if (indexLong == -1) { return 1; }
		return indexShort - indexLong;
	}
}
